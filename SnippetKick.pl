...
if ( $DupliOption == 0 ) {
	...
	close (FILE);
	system ("perl DupGrigoryanSpringborg.pl $threshold_duplicate tmpdupli.xyz");
	my @filesdupli = read_file("FinalCoords.xyz");
	#
	my $count  = 0;
	my $word_1 = "Isomer";
	my @linearray_1 = ();
...
for ( my $i=0; $i < $new_num_geo_out; $i++) {
	...
	if (($software=~/gaussian/gi )) {
		$option_soft = 1;
		my $G09name = G03Input ($without_extension,$header,$ncpus,$mem,$Charge,$Multiplicity,$final_coords[$i],$i);
...
# for gaussian
if (( $option_soft == 1 )) {
	my ($value_energy_sort,$value_coords_sort,$report_structure,$report_NImag,$file_opt) = submit_queue (\@files_tmp_gauss,$num_atoms_xyz,$header,$ncpus,$mem,$Charge,$Multiplicity,$queue,$exec_bin_g09);
